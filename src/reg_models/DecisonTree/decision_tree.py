...
def calculateDiffCount(datas):
    ...
    #     else:
    #         results[data[-1]] += 1
    results = dict(Counter(map(lambda x: x[-1], datas)))
    return results
...
def main():
    path = '/Users/longguangbin/Work/Codes/MLlearn/data/fishiris.csv'
    bHeader = True
    # the bigger example
    # dcHeadings, trainingData = loadCSV(path, bHeader)  # demo data from matlab
    fishiris = pd.read_csv(path)
    decisionTree = buildDecisionTree(fishiris.values.tolist(), evaluationFunction=gini)
    plot(decisionTree)
